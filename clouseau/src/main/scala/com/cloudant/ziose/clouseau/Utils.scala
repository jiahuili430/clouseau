...
object Utils {
  ...
  /**
   * The function `ensureElementsType` maps over elements of a list and applies a partial function `pf` to each element
   * until it encounters an element for which the `pf` is not defined. At that point, it stops iteration and calls the
   * `orElse` partial function passing the original `list` and the current element.
   *
   * This function is intended to be used to refine a generic list to a list containing specific type of elements. For
   * example, you can convert a `List[Any]` to `List[Inner]`, where `Inner` is a generic parameter of the function.
   *
   * @param pf
   *   \- `PartialFunction[Any, Inner]` - Partial function to apply to each element of the given list
   * @param orElse
   *   \- `PartialFunction[List[Any], Any), Inner]` - Partial function to apply to the first element for which `pf` is
   *   not defined.
   *
   ...
   * }
   * ```
   * In this example, the `asListOfStrings` partial function will convert any list to a `List[String]`, and throw an
   * exception if it encounters a non-string element.
   */

  def ensureElementsType[Inner](
    pf: PartialFunction[Any, Inner],
    orElse: PartialFunction[(List[Any], Any), List[Inner]]
  ): PartialFunction[Any, List[Inner]] = {
    case list: List[_] => {
  ...
  def clouseauNode: ZIO[EngineWorker & Node & ActorFactory & OTPNodeConfig, Throwable, ClouseauNode] = for {
    ...
    node <- ZIO.succeed(new ClouseauNode()(runtime, worker, metricsRegistry, metricsPrometheusRegistry, LogLevel.Debug))
  } yield node
