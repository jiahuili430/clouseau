 ...
 */
case class MetricsPrometheusGroup(klass: Class[_], metricsPrometheusRegistry: ScalangPrometheusMeterRegistry)
    extends ZioSupport {
  private val rateUnit       = metricsPrometheusRegistry.getRateUnit
  private val durationUnit   = metricsPrometheusRegistry.getDurationUnit
  ...
  final class Timer(codahaleTimer: metrics.Timer) {
    def time[A](fun: => A): A = codahaleTimer.time(new Callable[A] {
      def call = fun
    })
...
//    }
  ...
  //      "rateUnit"           -> metricsPrometheusRegistry.rateUnitName(rateUnit)
  //    )
  //  }
  //
  ...
  //      Symbol("rateUnit")           -> metricsPrometheusRegistry.rateUnitName(rateUnit)
  //    )
  //  }
  //
